let palavras = [
    ...
    { nome: "PIZZA", categoria: "comidas" }, { nome: "HAMBURGUER", categoria: "comidas" }, { nome: "LASANHA", categoria: "comidas" }, { nome: "SUSHI", categoria: "comidas" }, { nome: "CHURRASCO", categoria: "comidas" }, { nome: "SALADA", categoria: "comidas" }, { nome: "BATATAFRITA", categoria: "comidas" }, { nome: "MACARRAO", categoria: "comidas" }, { nome: "TACO", categoria: "comidas" }, { nome: "SOPA", categoria: "comidas" }, { nome: "OMELETE", categoria: "comidas" }, { nome: "BOLO", categoria: "comidas" }, { nome: "SANDUICHE", categoria: "comidas" }, { nome: "STROGONOFF", categoria: "comidas" }, { nome: "SORVETE", categoria: "comidas" }, { nome: "ARROZ", categoria: "comidas" }, { nome: "FEIJAO", categoria: "comidas" }, { nome: "TORTA", categoria: "comidas" }, { nome: "CACHORRO-QUENTE", categoria: "comidas" },
...
const somFalhaLetra = new Audio('sons/u_8g40a9z0la-fail-234710.mp3')
const somSucessoLetra = new Audio('sons/liecio-bonus-points-190035.mp3')
const vitorias = document.getElementById('contadorVitorias')
const derrotas = document.getElementById('contadorDerrotas')


btComecar.addEventListener("click", function (evento) {
    contadorCliques++
    if (contadorCliques > 1) {
        alert('Por favor, clique em Zerar para começar de novo')
        return
    }
    ...
    function soLetras(letra) {
        return /^[A - Za-z]$/.test(letra)
    }

    btEnviar.addEventListener("click", function enviarLetra(evento) {
        if (!soLetras(inputLetra.value)) {
            alert("Digite apenas uma letra maiúscula ou minúscula!")
            return
        }
        if (arrayDeLetras.includes(inputLetra.value.toUpperCase())) {
            letrasDigitadas.childNodes.forEach(function (letra) {
                if (letra.textContent === inputLetra.value.toUpperCase()) {
                    retorno = true
                } else {
                    retorno = false
                }
            ...
            letraDigitada.textContent = inputLetra.value.toUpperCase()
            letraDigitada.setAttribute('class', 'letrascertas')
            somSucessoLetra.play()
            ...
            const letraDigitada = document.createElement("li")
            letraDigitada.textContent = inputLetra.value.toUpperCase()
            letraDigitada.setAttribute('class', 'letraserradas')
            somFalhaLetra.play()
            letrasDigitadas.appendChild(letraDigitada)
            ...
            switch (imgsrc) {
                ...
                case "./imgs/perna1.png":
                    ...
                    contadorDerrotas++
                    derrotas.textContent = `Derrotas:${contadorDerrotas}`
                    somFalha.play()
        ...
        const naoTemLinha = arrayItens.every(i => i.textContent !== "__")
        const img = document.querySelector("img").getAttribute("src")
        if (naoTemLinha && img !== "./imgs/perna2.png") {
            document.getElementById("msgvitoria").textContent = "Você venceu!"
            contadorVitorias++
            vitorias.textContent=`Vitorias: ${contadorVitorias}`
            somSucesso.play()
        }
        inputLetra.value = ""
    })
